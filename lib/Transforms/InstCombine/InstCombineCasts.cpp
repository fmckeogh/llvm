...
Instruction *InstCombiner::visitTrunc(TruncInst &CI) {
  ...
  // Transform trunc(shr (sext A), Cst) to ashr A, Cst to eliminate type
  // conversion.
  ...
  if (Src->hasOneUse() &&
      match(Src, m_Shr(m_SExt(m_Value(A)), m_ConstantInt(Cst)))) {
    Instruction *SrcI = cast<Instruction>(Src);
    Value *SExt = SrcI->getOperand(0);
    ...
    // If Src is ashr, the bits are sign bits anyway.
    if (SExt->hasOneUse() &&
        (ShiftAmt <= SExtSize - ASize || SrcI->isArithmeticShift())) {
      // If shifting by size of the original value in bits or more, it is simply
      // filled with the sign bit, so shift by ASize-1 to avoid ub.
  ...
  if (Src->hasOneUse() &&
    ...
    const unsigned ASize = A->getType()->getPrimitiveSizeInBits();
    const unsigned ExtSize = cast<Instruction>(Src)->getOperand(0)->getType()
      ->getPrimitiveSizeInBits();
    // This optimization can be only performed when zero bits generated by
    // the original lshr aren't pulled into the value after truncation, so we
    // can only shift by values smaller than the difference between the
    // destination and source type (in bits).
    if (Cst->getZExtValue() <= ExtSize - ASize) {
